...
-(void)setHeaderView{
	...
	[_nameBtn setTitle:@"注册/登录" forState:UIControlStateNormal];
...
-(void)changeAvatar{
	...
	BmobObject *object = [BmobObject objectWithoutDataWithClassName:@"_User" objectId:user.objectId];
		...
		switch (index) {
			case 1:
			{
				[CAMERA openCamera:self edit:NO callback:^(UIImage *image) {
					...
					[file saveInBackground:^(BOOL isSuccessful, NSError *error) {
						if (isSuccessful) {
							[object setObject:file  forKey:@"img"];
			...
			case 2:
			{
				[CAMERA openGallery:self edit:NO callback:^(UIImage *image) {
					[_avartarBtn setImage:image forState:UIControlStateNormal];
					BmobFile *file = [[BmobFile alloc] initWithFileName:@"avatar.png" withFileData:UIImagePNGRepresentation(image)];
					[file saveInBackground:^(BOOL isSuccessful, NSError *error) {
						if (isSuccessful) {
							[object setObject:file  forKey:@"img"];
							[object updateInBackground];
...
-(void)updateProfile{
	...
	if (user) {
		...
		[query getObjectInBackgroundWithId:user.objectId block:^(BmobObject *object, NSError *error) {
			if (object) {
				BmobFile *file = (BmobFile *)[object objectForKey:@"img"];
				NSString *url = file.url;
				_avartarBtn.enabled = YES;
				[AJAX getImageWithUrl:url callback:^(BOOL isError, UIImage *image) {
					if (isError) {
						return ;
					}else{
						[_avartarBtn setImage:image forState:UIControlStateNormal];
