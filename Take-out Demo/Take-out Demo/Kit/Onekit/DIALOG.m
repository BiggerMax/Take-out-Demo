...
+ (void)alert:(NSString *)message callback:(void (^)(void))callback
{
    ...
    DIALOG.alertViewType = AlertViewTypeAlert;
     [self showAlert:Nil message:message leftButton:NSLocalizedString(@"close", nil) rightButton:Nil DIALOG:DIALOG];
   /* [[[UIAlertView alloc] initWithTitle:nil
                                message:message
                               delegate:DIALOG
...
+ (void)confirm:(NSString *)message callback:(void (^)(BOOL))callback
{
    ...
    [self confirm:message
         yesTitle:NSLocalizedString(@"yes", nil)
          noTitle:NSLocalizedString(@"no", nil)
         callback:callback];
}
