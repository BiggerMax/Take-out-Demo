...
@interface AJAX ()<NSURLSessionDelegate>
{
    NSURLSessionDownloadTask *task;
    NSString* _save;
    typeof (void (^)(CGFloat process))_callback2;
    typeof (void (^)(BOOL isError))_callback;
}
@end
...
//    for (NSString* api in APIs_GUEST) {
//        if(![url endWith:api]){
//            [request setValue:[CONFIG get2:@"token"] forHTTPHeaderField:@"Cookie"];
//        }else{
//            
//        }
//    }
    ...
    NSURLSessionDataTask* task = [session dataTaskWithRequest:request completionHandler:^(NSData *  data, NSURLResponse * response, NSError *  connectionError)
                                  {
                                      ...
                                      if(connectionError){
                                          ...
                                          if(statusCode!=200){
                                              if(statusCode==401){
                                                  [MESSAGE sendMessage:@"LOGIN" data:nil];
                                                  return;
                                              }
                                              switch (statusCode) {
                                                  case -1:
                                      ...
                                      if(message){
                                          dispatch_async(dispatch_get_main_queue(),^ {
                                              [DIALOG alert:[NSString stringWithFormat:@"%@\n%@",url,message] callback:^{
...
//                                      for (NSString* api in APIs_TOKEN) {
//                                          if([url endWith:api]){
//                                              NSString* token = httpResponse.allHeaderFields[@"Set-Cookie"];
//                                              NSString* str = [[NSString alloc]initWithData:data encoding:NSUTF8StringEncoding];
//                                              NSLog(@"================\n%@\n%@",str,httpResponse.allHeaderFields);
//                                              [CONFIG set2:@"token" value:token];
//                                          }
//                                      }
                                      dispatch_async(dispatch_get_main_queue(),^ {
             ...
             callback:(void (^)(BOOL isError,NSDictionary* json))callback
{
    ...
    for(int i = 0; i< [keys count] ; i++){
        //要上传的图片
        NSString* key = keys[i];
        NSData* file = [files objectForKey:key];
...
didFinishDownloadingToURL:(NSURL *)location {
    ...
    NSString* folder = [_save substringToIndex:[_save lastIndexOf:@"/"]];
    [mgr createDirectoryAtPath:folder withIntermediateDirectories:true attributes:nil error:nil];
    [mgr moveItemAtPath:location.path toPath:_save error:nil];
    _callback(FALSE);
}
